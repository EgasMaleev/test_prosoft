...
// ќпределение структуры дл€ элемента списка
typedef struct Node {
    void* data;
    struct Node* next;
} Node;

// —труктура дл€ представлени€ св€зного списка
typedef struct {
    Node* head;
} LinkedList;

// ‘ункци€ дл€ инициализации пустого списка
void initLinkedList(LinkedList* list) {
    list->head = NULL;
}

// ‘ункци€ дл€ добавлени€ нового элемента в конец списка
...
void addelem(LinkedList* list, Node* ptr, void* infa)
{
    ...
    p = ptr->next; // сохранение указател€ на следующий узел
    ptr->next = temp; // предыдущий узел указывает на создаваемый
    temp->data = infa; // сохранение пол€ данных добавл€емого узла
...
void deletelem(Node* lst, LinkedList* list)
{
    Node* temp;
    temp = list->head;
    while (temp->next != lst) // просматриваем список начина€ с корн€
    { // пока не найдем узел, предшествующий lst
        temp = temp->next;
    }
    temp->next = lst->next; // переставл€ем указатель
    free(lst); // освобождаем пам€ть удал€емого узла
}
...
void listprint(LinkedList* list)
{
    ...
    while (p != NULL)
    {
        printf("%d ", p->data); // вывод значени€ элемента p
...
// ѕример использовани€ API:
int main() {
    // »нициализаци€ нового св€зного списка
    LinkedList myList;
    initLinkedList(&myList);

    // ƒобавление элементов разного типа в список
    int intValue1 = 42;
    int intValue2 = 43;
    int intValue3 = 44;
    int intValue4 = 45;
